...
function afficherPlusieursFilms(tableauDeFilms) {
    tableauDeFilms.forEach(function (film) {
         ...
         elementFilms.setAttribute('id', film.name);
        ...
        var priceAndNameFilm = document.createElement('p');
        linkFilms.appendChild(priceAndNameFilm);

        var priceAndName = document.createTextNode(film.name + " " + film.prix + " " + film.description);
        priceAndNameFilm.appendChild(priceAndName);
    })};
...
//je rempli le texte dans la balise h1
var contentTitreFt = document.createTextNode("FADHW");
